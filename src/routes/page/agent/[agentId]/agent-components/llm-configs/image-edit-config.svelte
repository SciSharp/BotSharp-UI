<script>
    ...
    $: {
        if (llmConfigs.length > 0 && innerLlmConfigs.length === 0) {
            ...
            const innerProviders = innerLlmConfigs.filter(x => x.models?.some(y => y.capabilities?.includes(LlmModelCapability.ImageEdit)));
    ...
    function getLlmModels(provider) {
        return innerLlmConfigs.find(x => x.provider === provider)?.models
                             ?.filter(x => x.capabilities?.includes(LlmModelCapability.ImageEdit)) || [];
    }
...
<div class="agent-config-container">
    <div class="text-center">
        <h6 class="mt-1 mb-2">Image Edit</h6>
    </div>
